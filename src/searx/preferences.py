...
class Setting:
    ...
    def parse_form(self, data):  # added to satisfy type check when called on base type
        if self.locked:
            return
    ...
    def __init__(
        ...
        self.key_value_settings: Dict[str, Setting] = {
            ...
            'favicon_resolver': EnumStringSetting(
                ...
                choices=(list(favicons.proxy.CFG.resolver_map.keys()) + ['']) if getattr(favicons.proxy, 'CFG', None) else ['']
            ),
