...
class Search:
    ...
    def search_multiple_requests(self, requests):
        ...
        for engine_name, query, request_params in requests:
            # Wrap with Flask request context only if available; otherwise call directly
            search_callable = PROCESSORS[engine_name].search
            try:
                if has_request_context():
                    search_callable = copy_current_request_context(search_callable)
            except Exception:
                pass
            th = threading.Thread(  # pylint: disable=invalid-name
                target=search_callable,
